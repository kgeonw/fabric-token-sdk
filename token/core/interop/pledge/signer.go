...
// PledgeSigner for a pledge script
type PledgeSigner struct {
...
// PledgeSignature encodes the signature that spends a pledge script
type PledgeSignature struct {
	Reclaim bool
...
func (s *PledgeSigner) Sign(message []byte) ([]byte, error) {
	sigma := PledgeSignature{}
	var err error
	if s.Issuer == nil {
		if s.Sender == nil {
...
// PledgeVerifier of a PledgeSignature it is uniquely linked to a the pledge script identified by
// PledgeID
...
func (v *PledgeVerifier) Verify(message, sigma []byte) error {
	sig := &PledgeSignature{}
